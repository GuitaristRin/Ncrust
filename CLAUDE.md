...
- **Borderless list style (Groove)**: `SongCard` LIST/COMPACT rows have `0dp` left padding — the 72dp cover is flush to the screen edge. Only the right side keeps 16dp padding for text. `DetailScaffold` has no M3 `TopAppBar` Surface — the back arrow floats over the content with a semi-transparent black square backdrop. `DetailHeader` uses a full-width cover (`fillMaxWidth().aspectRatio(1f)`) with title/subtitle/info stacked below. Grid tiles use `spacedBy(2.dp)` for the "Metro tile wall" feel; text under a tile gets `padding(horizontal = 6.dp)` to avoid touching the screen edge. Home and Library screens use a 34sp Regular page header (`statusBarsPadding()` + start 16dp) instead of an M3 `TopAppBar`.
